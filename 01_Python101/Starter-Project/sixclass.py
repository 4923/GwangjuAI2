...
Original file is located at
    https://colab.research.google.com/drive/1uJkFrdNI9RadT1FweEEovbX95gGBklzh
"""
...
class Six_Class:
    ...
    def age_gender(self):
        return "{}세 / {} 이신 고객님의 전기 사용량({}kWh)은 비슷한 연령대 고객님들보다 높습니다.".format(
            self.age, self.gender, self.kWh
        )

    def gender_phone(self):
        return "{} 고객님 연락하신 휴대번호 {} 로 스마트 이체를 강제로 신청해 드리겠습니다.".format(
    ...
    def kWh_email(self):
        ...
        return "{} 고객님 금월 우리 계좌로 이체 해주셔야 하는 금액은 {} 원입니다. 안내시면 고발합니다.".format(
            self.name, format(int(result_cost), ",")
        )
    ...
    def send_mail(self):
        ...
        s.login(self.email, "ubswbnujtogpwzdt")

        txt = "{}\n{}\n{}\n{}".format(
            self.name_age(), self.age_gender(), self.gender_phone(), self.kWh_email()
        )
        msg = MIMEText(txt)
