...
class Commerce6_product:
    ...
    def secure():
        ...
        )  # 영어 소문자부터 대문자까지 생성(abc ... XYZ) string.ascii_lowercase, string.ascii_uppercase
    ...
    def product_infomation_resule(self):
        ...
        login_df = pd.read_csv(
            "C:/Users/wjsdn/Desktop/woong/Commerce6_login.csv", encoding="euc-kr"
        )
        product_df = pd.read_csv(
            "C:/Users/wjsdn/Desktop/woong/bestsellers with categories.csv"
        )
        ...
        while True:
            ...
            product_choice_options = int(
                input(
                    "평점높은순(1) / 리뷰많은순(2) / 낮은가격순(3) / 높은가격순(4) / 최신출시순(5) / 장르별(6) / 장바구니 확인(7): "
                )
            )

            if product_choice_options == 1:
                product_df = product_df.sort_values(
                    by=["User Rating"], axis=0, ascending=False
                )
            elif product_choice_options == 2:
                product_df = product_df.sort_values(
                    by=["Reviews"], axis=0, ascending=False
                )
            elif product_choice_options == 3:
                product_df = product_df.sort_values(
                    by=["Price"], axis=0, ascending=True
                )
            elif product_choice_options == 4:
                product_df = product_df.sort_values(
                    by=["Price"], axis=0, ascending=False
                )
            elif product_choice_options == 5:
                product_df = product_df.sort_values(
                    by=["Year"], axis=0, ascending=False
                )
                ...
                if select_Genre == 1:
                    ...
                    select_rating = input(
                        "평점높은순(1) / 리뷰많은순(2) / 낮은가격순(3) / 높은가격순(4) / 최신출시순(5) : "
                    )
                    if select_rating == "1":
                        product_df = product_df.sort_values(
                            by=["User Rating"], axis=0, ascending=False
            ...
            for i in range(5):
                ...
                for j in range(len(product_df.columns)):
                    if j == 4:
                        print(
                            "{} : {} 원".format(
                                product_df.columns.to_list()[j],
                                format(product_df.loc[i][j] * 1000, ","),
                            )
                        )
                    else:
                        print(
                            "{} : {}".format(
                                product_df.columns.to_list()[j], product_df.loc[i][j]
                            )
                        )
                print("-------------------------------")
            ...
            print(
                "{} 님, 구매 신청하신 서적은 {} 입니다.".format(
                    login_df.loc[
                        (login_df["id"] == self.id) & (login_df["pw"] == self.pw)
                    ]["nickname"].values[0],
                    product_df.loc[choice_product_booknumber - 1][0],
                )
            )
            print(
                "가격은 {} 원입니다.".format(
                    product_df.loc[choice_product_booknumber - 1][4] * 1000
                )
            )
            ...
            if choice_product_bookorder == 1:
                ...
                with open(
                    "C:/Users/wjsdn/Desktop/woong/my_cart.csv", "a", newline=""
                ) as file:
                    writer = csv.writer(file)
                    writer.writerow(data_list)
                print(
                    "{}님이 선택하신 책 {}({})가 장바구니에 담겼습니다.\n".format(
                        self.id,
                        product_df.loc[choice_product_booknumber - 1][0],
                        product_df.loc[choice_product_booknumber - 1][1],
                    )
                )
            if break_number2 == 1:
                break
    ...
    def product_receipt(self):
        ...
        cartedit_df = pd.read_csv(
            "C:/Users/wjsdn/Desktop/woong/my_cartedit.csv",
            header=None,
            names=["Title", "Price"],
            usecols=[1, 2],
        )
        ...
        if receipt_string == "Y":
            ...
            print(
                "상기 금액을 {}년 {}월 {}일 수령하고 위 금액의 수령을 증표로서 본 영수증을 발행합니다.".format(
            ...
            pyautogui.screenshot(
                "C:/Users/wjsdn/Desktop/woong/reciept.png", region=(115, 737, 1500, 200)
            )
        ...
        print("배송정보")
        print(
            "{}님이 구매 신청하신 서적 {}등 {}개가 {}로 3일내로 도착 예정입니다.".format(
                login_df.loc[(login_df["id"] == self.id) & (login_df["pw"] == self.pw)][
                    "nickname"
                ].values[0],
                cartedit_df["Title"][0],
                len(cartedit_df),
...
class Commerece6_purchase:
    ...
    def cart_look(self):
        ...
        cart_df = pd.read_csv(
            "C:/Users/wjsdn/Desktop/woong/my_cart.csv",
            header=None,
            names=["Title", "Price"],
            usecols=[1, 2],
        )
    ...
    def all_purchase(self):
        ...
        with open("C:/Users/wjsdn/Desktop/woong/my_cart.csv", "r") as inp, open(
            "C:/Users/wjsdn/Desktop/woong/my_cartedit.csv", "w", newline=""
        ) as out:
    ...
    def choice_purchase(self):
        ...
        if Commerce6_product.secure():
            ...
            time.sleep(3)
            print(
                "{}님, 선택하신 책의 총 {}원 구매가 완료되었습니다.".format(
                    login_df.loc[
                        (login_df["id"] == self.id) & (login_df["pw"] == self.pw)
                    ]["nickname"].values[0],
                    total_price,
