...
class board_db:
    ...
    def start(self):
        while True:
            state = int(
                input("항목을 선택해주세요 [ 리뷰보기(1), 리뷰쓰기(2), 리뷰검색(3), 리뷰수정(4), 종료(5) ] :")
            )
            if state == 1:
    ...
    def write_board(self):
        ...
        board_df = pd.read_csv(
            "C:/Users/wjsdn/Desktop/woong/Commerce6_board.csv", encoding="euc-kr"
        )
        print(product_df)
        # print(board_df)
        my_post = []
        ...
        board_df.to_csv(
            "C:/Users/wjsdn/Desktop/woong/Commerce6_board.csv",
            index=False,
            encoding="euc-kr",
        )
        ### 수정 삭제 비밀번호 입력하기
